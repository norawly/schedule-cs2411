...
export async function runCron(env) {
  ...
  await db.setMeta(env, "cron_last", new Date().toISOString());      // видно, что cron жив
...
export async function ensureWebhook(env, force = false) {
  ...
  if (!force && await db.getMeta(env, "webhook") === want) return { ok: true, cached: true };
  ...
  const r = await tg(env, "setWebhook", {
    ...
    drop_pending_updates: !force,               // вручную — не теряем сообщения в очереди
  });
  if (!r.ok) {
