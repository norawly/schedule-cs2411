...
/* ---------- состояние ---------- */
var S = {data:null, floor:1, sel:null, tool:"select", view:"both", dirty:false,
...
function renderPlan(){
  ...
  f.shapes.forEach(function(s){
    var el=svg(s.closed?"polygon":"polyline",{
      points:s.p.map(function(p){return p.join(",");}).join(" "),
      "class":"sh"+(s.closed?" closed":"")});
...
function select(r,scroll){
  S.sel=r; renderList(); renderPlan(); renderProps(); highlight3D();
  if(scroll){
